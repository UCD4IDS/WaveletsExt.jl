...
@testset "Denoise" begin
    ...
    y = denoise(dwt(x, wt, 4), :dwt, wt, L=4)
    @test relativenorm(y, x0) <= err
    y = denoise(dwt(x, wt), :dwt, wt)
    @test relativenorm(y, x0) <= err
    y = denoise(wpt(x, wt), :wpt, wt, tree=maketree(x, :full))
    @test relativenorm(y, x0) <= err
    ...
    y = denoiseall(x, :sig, wt, bestTH=mean)
    @test mean([relativenorm(y[:,i],x0[:,i]) for i in 1:5]) <= max_err
    y = denoiseall(hcat([dwt(x[:,i], wt) for i in 1:5]...), :dwt, wt)
    @test mean([relativenorm(y[:,i],x0[:,i]) for i in 1:5]) <= max_err
